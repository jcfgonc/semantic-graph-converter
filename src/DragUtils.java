...
public class DragUtils {
	...
	private static void convertFiles(List<File> list, String outputFormat, JFrame mainFrame) {
		...
		for (File f : list) {
			if (f.isFile()) {
				convertFile(f.getAbsolutePath(), outputFormat);
				convertedFiles++;
			} else {
	...
	private static void convertFile(String filename, String outputFormat) {
		try {
			StringGraph graph = GraphReadWrite.readAutoDetect(filename);
